...
export default function PartyBanner() {
  ...
  // 🔥 Ganti ini dengan URL API dari panelmu
  const API_URL = "https://your-api-domain.com/api/banners";
  ...
  const fetchBanners = async () => {
    try {
      ...
      // Download dan cache gambar ke lokal storage
      const cached = await Promise.all(
        json.map(async (item) => {
          ...
          if (!info.exists) {
            await FileSystem.downloadAsync(item.image, localUri);
          }
