...
export default function EwalletPaymentScreen({ navigation, route }) {
  const ewalletInfo = route.params?.ewalletInfo || { name: "E-Wallet", logo: null };
  ...
  const fetchEwalletPackages = async () => {
    try {
      const response = await api.get("/payment/packages");
      if (response.data.success) {
        // Filter ewallet packages (id >= 100)
        const ewalletPkgs = (response.data.data.ewallet || []).filter(pkg => pkg.id >= 100);
        setPackages(ewalletPkgs);
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>{ewalletInfo.name}</Text>
