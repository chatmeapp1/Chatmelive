...
export default function AsiaPartyTab() {
  ...
  const fetchAsiaRooms = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/api/party/asia`);
      if (response.data.success) {
        setData(response.data.data);
      }
    } catch (error) {
      console.error("❌ Error fetching asia party rooms:", error);
    } finally {
      setLoading(false);
    }
  };
