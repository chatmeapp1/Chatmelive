...
export default function MiddleEastPartyTab() {
  const navigation = useNavigation();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState([]);

  useEffect(() => {
    fetchMiddleEastRooms();
  }, []);

  const fetchMiddleEastRooms = async () => {
    try {
      setLoading(true);
      const url = `${API_URL}/api/party/middle-east`;
      console.log("📡 Fetching Middle East rooms from:", url);
      const response = await axios.get(url);
      if (response.data.success) {
        setData(response.data.data);
        console.log("✅ Middle East rooms loaded:", response.data.data.length);
      }
    } catch (error) {
      console.error("❌ Error fetching middle east party rooms:", error.message);
